...
class CNN(object):
    ...
    def __init__(self, dtype="fp32", scope_name="", is_inference=False,
        ...
        # Hyper-parameters
        self.__K = 32      # Conv-1 depth
        self.__stride1 = 1 # Conv-1 stride
        self.__L = 64      # Conv-2 depth
        self.__stride2 = 1 # Conv-2 stride
        self.__N = 1024     # FC width
    ...
    def __add_variables(self):
        ...
        # Conv-1 weights
        self.__W1 = tf.Variable(tf.truncated_normal([5, 5, 1, self.__K],
        ...
        # FC weights
        self.__W4 = tf.Variable(tf.truncated_normal([7 * 7 * self.__L, self.__N],
                                                    stddev=0.1, dtype=self.__dtype))
    ...
    def __build_graph(self, X):
        ...
        Y4r = tf.nn.relu(Y4bn)
        Y4 = tf.nn.dropout(Y4r, 0.4)

        Ylogits = tf.matmul(Y4, self.__W5) + self.__B5
        Y = tf.nn.softmax(Ylogits)
        update_ema = tf.group(update_ema1, update_ema2, update_ema4)
